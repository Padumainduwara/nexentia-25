  ...
  useLayoutEffect(() => {
    const ctx = gsap.context(() => {
      ...
      tl.from(".comp-hero-logo", { scale: 0.5, opacity: 0, duration: 1, ease: "power3.out" })
        .from(".anim-fade-in", { y: 50, opacity: 0, duration: 0.6, stagger: 0.1, ease: "power3.out" }, "-=0.7");
    }, compRef);
    return () => ctx.revert();
  }, []);

  // Check for the special 'most-popular' slug
  ...
  return (
    <div ref={compRef} className="w-full bg-black font-orbitron pt-32 pb-16 min-h-screen overflow-hidden">
      <div className="container mx-auto px-6 md:px-10">
        ...
        {isMostPopularAward ? (
          // Special layout for "Most Popular ICT Society"
          <div className="text-center max-w-2xl mx-auto">
            <p className="anim-fade-in font-sans text-lg text-gray-300 leading-relaxed mb-12">
              This special award recognizes the ICT Society that garners the most support from the community. Vote for your favorite school and watch them climb the ranks on the live leaderboard!
            </p>
            <div className="anim-fade-in flex flex-col sm:flex-row gap-6 justify-center">
              <button className="px-8 py-3 bg-transparent border-2 border-purple-500 text-purple-500 font-semibold rounded-lg hover:bg-purple-500 hover:text-white transition-all duration-300 text-lg" data-cursor-hover>
                R & R
              </button>
          ...
          // Standard layout for competitions with sub-categories
          ...
          // Fallback for competitions with no sub-categories (besides the award)
